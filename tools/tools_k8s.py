...
K8S_TOOLS["get_unhealthy_pods_detail"] = {
    ...
    "description": (
        ...
        "Restart counts: the output includes restart count per pod — for questions like 'pods restarted "
        "more than 5 times' or 'high restart count', call this tool and filter the output mentally "
        "to only report pods meeting the threshold. "
...
K8S_TOOLS["exec_db_query"] = {
    ...
    "description": (
        ...
        "DIALECT RETRY: If you call with MySQL SQL (e.g. SELECT user FROM mysql.user) and get an error "
        "saying relation does not exist, the DB is PostgreSQL — immediately re-call with the PostgreSQL "
        "equivalent (e.g. SELECT usename, passwd FROM pg_shadow WHERE usename='x'). "
        "Never give up after one dialect failure — always retry with the other dialect. "
